...
function FG_UI_GutterPanel:render()
    ...
    self:drawText(getText("IGUI_panel_FunctionalGutters_panel_title"), x, y, 1, 1, 1, 1, UIFont.Medium)
end
...
function FG_UI_GutterPanel:onButton(_btn)
    ...
    elseif _btn.internal == "BUILD" then
        ...
        local searchFilter = getText("IGUI_craft_FunctionalGutters_search_filter")
...
function FG_UI_GutterPanel:styleToggleButton()
    ...
    if not self.collector or not self.primaryCollector then
        self.btnToggleConnect.title = getText("ContextMenu_FunctionalGutters_ConnectContainer")
        self.btnToggleConnect.tooltip = getText("IGUI_panel_FunctionalGutters_section_Collector_icon_tooltip_error")
    ...
    if self.canPlumb then
        ...
        if not utils:getModDataIsGutterConnected(self.primaryCollector) then
            ...
            self.btnToggleConnect.title = getText("ContextMenu_FunctionalGutters_DisconnectContainer")
...
function FG_UI_GutterPanel:DoConnectCollector()
    ...
    self.btnToggleConnect.title = getText("IGUI_panel_FunctionalGutters_section_Collector_btn_connecting")
...
function FG_UI_GutterPanel:DoDisconnectCollector()
    ...
    -- This will be swapped for primaryCollector inside the method if the object is a multi-tile trough
    if not self:checkCanPlumb() or not self.collector or self.disableConnect then return end
    self.btnToggleConnect.title = getText("IGUI_panel_FunctionalGutters_section_Collector_btn_disconnecting")
